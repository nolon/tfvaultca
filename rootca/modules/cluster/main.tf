...
resource "vault_pki_secret_backend_intermediate_cert_request" "etcd_ca" {
...
resource "vault_pki_secret_backend_root_sign_intermediate" "etcd_ca" {
  depends_on = [ vault_pki_secret_backend_intermediate_cert_request.etcd_ca ]
  backend = var.vault_mount_root
  for_each = var.clusters
  csr = vault_pki_secret_backend_intermediate_cert_request.etcd_ca[each.key].csr
  common_name          = "etcd-ca"
...
resource "vault_pki_secret_backend_intermediate_cert_request" "k8s_ca" {
